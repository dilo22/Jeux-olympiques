...
class Epreuves:
    ...
    def _build_ui(self):
        ...
        self.filter_frame = tk.Frame(self.root, bg=self.BG)
        self.filter_frame.pack(fill="x", padx=20, pady=(0, 10))

        tk.Frame(self.root, bg=self.CARD_BORDER, height=1).pack(fill="x", padx=20, pady=(0, 10))
    ...
    def _build_filters(self):
        ...
        # Row 0
        tk.Label(self.filter_frame, text="Date :", bg=self.BG, fg=self.SUB_FG)\
            .grid(row=0, column=0, padx=(0, 6), pady=6, sticky="w")

        dates = [f"2024-07-{day:02d}" for day in range(24, 32)] + [f"2024-08-{day:02d}" for day in range(1, 12)]
        self.date_combobox = ttk.Combobox(self.filter_frame, values=dates, state="readonly", width=14)
        self.date_combobox.set(dates[0] if dates else "")
        self.date_combobox.grid(row=0, column=1, padx=(0, 16), pady=6, sticky="w")

        tk.Label(self.filter_frame, text="Équipe :", bg=self.BG, fg=self.SUB_FG)\
        ...
        self.equipe_combobox.grid(row=0, column=3, padx=(0, 16), pady=6, sticky="w")

        tk.Label(self.filter_frame, text="Sport :", bg=self.BG, fg=self.SUB_FG)\
            .grid(row=0, column=4, padx=(0, 6), pady=6, sticky="w")
        ...
        self.sport_combobox.grid(row=0, column=5, padx=(0, 0), pady=6, sticky="w")

        # Row 1
        tk.Label(self.filter_frame, text="Genre :", bg=self.BG, fg=self.SUB_FG)\
            .grid(row=1, column=0, padx=(0, 6), pady=6, sticky="w")

        genres = ["Tous genres", "Homme", "Femme", "Mixte"]
        self.genre_combobox = ttk.Combobox(self.filter_frame, values=genres, state="readonly", width=14)
        self.genre_combobox.set("Tous genres")
        self.genre_combobox.grid(row=1, column=1, padx=(0, 16), pady=6, sticky="w")

        tk.Label(self.filter_frame, text="Phase :", bg=self.BG, fg=self.SUB_FG)\
            .grid(row=1, column=2, padx=(0, 6), pady=6, sticky="w")

        phases = ["Toutes les phases", "Médaille"]
        self.phase_combobox = ttk.Combobox(self.filter_frame, values=phases, state="readonly", width=22)
        self.phase_combobox.set("Toutes les phases")
        self.phase_combobox.grid(row=1, column=3, padx=(0, 16), pady=6, sticky="w")
        ...
        self.search_btn.grid(row=1, column=5, padx=(0, 0), pady=6, sticky="e")

        # Auto refresh optionnel
        self.date_combobox.bind("<<ComboboxSelected>>", lambda _e: self.filtrer_donnees())

    # ---------------- Scrollable list ----------------
    def _build_list(self):
        self.canvas = tk.Canvas(self.root, bg=self.BG, highlightthickness=0)
        self.canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(20, 0), pady=(0, 10))
    ...
    def _add_event_card(self, nom_evenement: str, phase: str):
        ...
        card.bind("<Enter>", lambda _e: card.configure(highlightbackground="#C7D2FE"))
    ...
    def _to_db_value(self, s: str):
        """Convertit les valeurs 'Tous/Toutes ...' en None pour ne pas filtrer."""
        if s is None:
            return None
        v = str(s).strip()
        if not v:
        ...
        if low.startswith("tous ") or low.startswith("toutes ") or low in ("tous", "toutes"):
            return None
        if low in ("tous genres", "toutes genres", "toutes les phases", "tous les sports", "toutes les équipes"):
            return None
        return v

    def _fetch_epreuves(self, date, equipe, sport, genre, phase):
        """
        Appelle fetch_epreuves_filtrees en s’adaptant à ta DB.
        - Si ta fonction accepte 5 args => OK
        - Sinon 1 arg (date) => fallback
        """
        fn = self.db_manager.fetch_epreuves_filtrees

        # Essais du plus complet au plus simple
        for args in [
    ...
    def filtrer_donnees(self):
        ...
        try:
            ...
            # si la DB plante, on affiche une erreur claire
    ...
    def open_participant_window(self, id_match):
        ...
        for i, t in enumerate(labels):
            tk.Label(header, text=t, bg=self.BG, fg=self.TITLE_FG, font=("Helvetica", 11, "bold"))\
        ...
        for p in participants:
            ...
            row = tk.Frame(frame, bg=self.CARD_BG, highlightthickness=1, highlightbackground=self.CARD_BORDER)
            ...
            tk.Label(row, text=str(participant), bg=self.CARD_BG, fg=self.TITLE_FG, font=("Helvetica", 10, "bold"))\
                .grid(row=0, column=0, sticky="w", padx=10, pady=8)
            tk.Label(row, text=str(ph), bg=self.CARD_BG, fg=self.SUB_FG, font=("Helvetica", 10))\
                .grid(row=0, column=1, sticky="w", padx=6)
            tk.Label(row, text=str(res), bg=self.CARD_BG, fg=self.SUB_FG, font=("Helvetica", 10))\
                .grid(row=0, column=2, sticky="w", padx=6)
            tk.Label(row, text=str(med), bg=self.CARD_BG, fg=self.SUB_FG, font=("Helvetica", 10))\
